...
namespace Musicefy.Converters
{
    public class IconGlyphConverter : IValueConverter
    {
        ...
        private static readonly Lazy<Dictionary<string, IMusicSourceProvider>> _providerCacheLazy =
            new Lazy<Dictionary<string, IMusicSourceProvider>>(() =>
            {
                try
                {
                    var providers = App.Services.GetServices<IMusicSourceProvider>();
                    var cache = new Dictionary<string, IMusicSourceProvider>(StringComparer.OrdinalIgnoreCase);
                    foreach (var p in providers)
                    {
                        if (!cache.ContainsKey(p.SourceType))
                            cache[p.SourceType] = p;
                    }
                    return cache;
                }
                catch
                {
                    return new Dictionary<string, IMusicSourceProvider>(StringComparer.OrdinalIgnoreCase);
                }
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            if (value is string key)
            {
                ...
                var providers = _providerCacheLazy.Value;
